    ...
    return (
        <div className="w-full text-slate-800 dark:text-slate-200">
            <section className="max-w-7xl mx-auto px-6 py-12">
                ...
                <div className="bg-slate-900 text-white rounded-3xl p-6 sm:p-8 shadow-card space-y-4 mb-12 relative overflow-hidden border border-slate-800">
                ...
                <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
                    ...
                    <div className="w-full lg:w-2/3 space-y-10">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-6 gap-6 select-none">
                            {/* Categories */}
                                    ...
                                    <button
                                        ...
                                        className={`px-4 py-2 text-xs font-bold uppercase tracking-wider rounded-lg border transition-all cursor-pointer ${
                                            selectedCategory === category
                                                ...
                                                : 'bg-white dark:bg-[#131c31] border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:border-slate-300 dark:hover:border-slate-700'
                                        }`}
                                    >
                                        {category}
                                    </button>
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                                <span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Sort by:</span>
                                <select 
                                    value={sortBy}
                                    onChange={(e) => setSortBy(e.target.value)}
                                    className="bg-white dark:bg-[#131c31] border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-xs font-bold text-slate-800 dark:text-white outline-none focus:border-slate-400"
                                >
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-xs text-slate-400 dark:text-slate-500 font-medium">100% Annual Frequency</span>
                            </div>

                            {filteredPublications.length === 0 ? (
                                <div className="p-12 text-center bg-slate-50 dark:bg-[#131c31] rounded-2xl border border-slate-100 dark:border-slate-800">
                                    <p className="text-sm text-slate-400 dark:text-slate-500 font-medium">No publications found matching this category filter.</p>
                                </div>
                            ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {filteredPublications.map(pub => (
                                        <div 
                                            key={pub.id} 
                                            className="bg-white dark:bg-[#131c31] border border-slate-200/80 dark:border-slate-800/80 rounded-2xl p-5 shadow-xs flex gap-5 hover:shadow-card transition-all duration-300 relative overflow-hidden group"
                                        >
                                            ...
                                            <div className="flex flex-col justify-between flex-1 select-none">
                                                <div>
                                                    <h4 className="font-extrabold text-slate-900 dark:text-white text-sm mb-1 leading-snug">{pub.name}</h4>
                                                    <span className="text-[9px] font-black uppercase tracking-wider text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-[#18233c] border border-slate-200 dark:border-slate-700 px-2 py-0.5 rounded w-fit block mb-2">
                                                        {pub.publisher}
                                                    </span>
                                                    <p className="text-xs text-slate-500 dark:text-slate-300 font-light leading-relaxed line-clamp-3 mb-4">{pub.description}</p>
                                                </div>
                    ...
                    <div className="w-full lg:w-1/3 space-y-8 select-none">
                        ...
                        {/* Information Note */}
                        <div className="bg-blue-50/70 dark:bg-blue-950/30 border border-blue-100 dark:border-blue-900/40 rounded-3xl p-6 space-y-3">
                        ...
                        <div className="bg-white dark:bg-[#131c31] border border-slate-200/80 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-card space-y-6">
                            <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-4">
                                <h3 className="text-sm font-black uppercase tracking-widest text-slate-900 dark:text-white">Archived Annual Issues</h3>
                            </div>

                            <div className="space-y-4">
                                {PREVIOUS_ISSUES.map((issue, idx) => (
                                    <div key={idx} className="flex gap-4 p-3 hover:bg-slate-50 dark:hover:bg-[#18233c] rounded-xl transition-colors group border border-slate-100 dark:border-slate-800">
                                        {/* Cover */}
                                        ...
                                        <div className="flex flex-col justify-center">
                                            <h4 className="font-extrabold text-slate-900 dark:text-white text-xs line-clamp-1 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{issue.name}</h4>
                                            <span className="text-[10px] text-slate-400 dark:text-slate-500 font-bold mb-2">{issue.volume}</span>
                                            <a 
                                                href={issue.downloadUrl} 
                                                target="_blank" 
                                                rel="noopener noreferrer" 
                                                className="inline-flex items-center gap-1 text-[10px] font-black text-blue-600 dark:text-blue-400 uppercase tracking-wider hover:text-blue-800 dark:hover:text-blue-300 transition-colors"
                                            >
