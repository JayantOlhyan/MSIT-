        ...
        <main className="min-h-screen bg-slate-50 dark:bg-[#0a0f1d] transition-colors duration-300">
            ...
            <section className="py-12 sm:py-16 max-w-7xl mx-auto px-4 sm:px-6">
                <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
                    ...
                    <h2 className="text-3xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tight">Faculty Coordinators</h2>
                    <p className="text-slate-500 dark:text-slate-400 text-sm mt-3 font-semibold">Distinguished professors providing core oversight, requirements management, and academic compliance for the project.</p>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-6 sm:gap-8 max-w-4xl mx-auto">
                    {teamData.teachers.map((member) => (
                        <div 
                            ...
                            className="bg-white dark:bg-[#131c31] rounded-3xl p-5 sm:p-6 border border-slate-200 dark:border-slate-800/80 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer group flex flex-row gap-4 sm:gap-6 items-center"
                        >
                            <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden shrink-0 ring-2 ring-slate-100 dark:ring-slate-700">
                                <img src={member.img} alt={member.name} className="w-full h-full object-cover" />
                            </div>
                            <div className="flex-1 min-w-0">
                                <span className="inline-block px-2 py-0.5 bg-blue-50 dark:bg-blue-950/50 text-primary dark:text-blue-300 text-[10px] font-black uppercase tracking-wider rounded mb-1">{member.dept}</span>
                                <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white group-hover:text-primary transition-colors truncate">{member.name}</h3>
                                <p className="text-slate-500 dark:text-slate-400 text-xs font-semibold line-clamp-2 mt-0.5">{member.role}</p>
            ...
            {/* STUDENTS SECTION */}
            <section className="py-12 sm:py-16 max-w-7xl mx-auto px-4 sm:px-6 border-t border-slate-200 dark:border-slate-800/80">
                <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16">
                    <span className="text-blue-600 dark:text-blue-400 font-black tracking-[0.2em] text-xs uppercase mb-3 block">Engineering & Design</span>
                    <h2 className="text-3xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tight">Student Developers</h2>
                    <p className="text-slate-500 dark:text-slate-400 text-sm mt-3 font-semibold">Core students from Computer Science & Information Technology engineering batches building and optimizing the platform.</p>
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-6 sm:gap-8">
                    {teamData.students.map((member) => (
                        <div 
                            key={member.id}
                            onClick={() => setSelectedMember(member)}
                            className="bg-white dark:bg-[#131c31] rounded-3xl p-5 sm:p-6 border border-slate-200 dark:border-slate-800/80 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer group flex flex-col justify-between"
                        >
                            <div>
                                <div className="flex gap-4 items-center mb-4 sm:mb-6">
                                    <div className="w-16 h-16 rounded-full overflow-hidden shrink-0 ring-2 ring-slate-100 dark:ring-slate-700">
                                        <img 
                                            src={member.img} 
                                            alt={member.name} 
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <span className="inline-block px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[10px] font-black uppercase tracking-wider rounded mb-1">{member.dept}</span>
                                        <h3 className="text-base font-bold text-slate-900 dark:text-white group-hover:text-primary transition-colors truncate">{member.name}</h3>
                                        <p className="text-slate-500 dark:text-slate-400 text-xs font-semibold line-clamp-2">{member.role}</p>
                                    </div>
                                </div>
                                <p className="text-slate-500 dark:text-slate-400 text-xs font-medium leading-relaxed line-clamp-3 mb-5 sm:mb-6">
                                    {member.bio}
                                </p>
                            </div>

                            <div className="border-t border-slate-100 dark:border-slate-800/80 pt-4 flex justify-between items-center text-primary font-bold text-xs">
                                <span>Explore Contributions</span>
                                <div className="w-6 h-6 rounded-full bg-blue-50 dark:bg-blue-950/50 flex items-center justify-center group-hover:bg-primary group-hover:text-white transition-all shrink-0">
            ...
            {selectedMember && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-6 animate-fade-in">
                    ...
                    {/* Modal Card */}
                    <div className="relative bg-white dark:bg-[#131c31] w-full max-w-2xl max-h-[90vh] overflow-y-auto scrollbar-hide rounded-3xl sm:rounded-[2rem] shadow-2xl border border-slate-200 dark:border-slate-800 animate-scale-in">
                        {/* Close button */}
                        <button 
                            onClick={() => setSelectedMember(null)}
                            className="absolute top-4 right-4 sm:top-6 sm:right-6 w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 flex items-center justify-center text-slate-700 dark:text-slate-200 hover:text-slate-900 dark:hover:text-white transition-colors z-20"
                        >
                            <X className="w-5 h-5" />
                        </button>
                        ...
                        {/* Details body */}
                        <div className="p-5 sm:p-8 space-y-6 sm:space-y-8 bg-[#f8fafc] dark:bg-[#0c1322]">
                            <div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                <div className="bg-white dark:bg-[#18233c] p-3.5 sm:p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">Qualification</div>
                                    <div className="text-xs font-extrabold text-slate-800 dark:text-white">{selectedMember.qualifications}</div>
                                </div>
                                <div className="bg-white dark:bg-[#18233c] p-3.5 sm:p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1">Experience / Status</div>
                                    <div className="text-xs font-extrabold text-slate-800 dark:text-white">{selectedMember.experience}</div>
                                </div>
                            </div>

                            <div>
                                <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-1.5 border-b border-slate-200 dark:border-slate-800 pb-2">
                                ...
                                <ul className="space-y-3">
                                    {selectedMember.contributions.map((item, idx) => (
                                        <li key={idx} className="flex gap-2 items-start text-xs font-semibold text-slate-600 dark:text-slate-300">
                            ...
                            <div className="border-t border-slate-200 dark:border-slate-800 pt-6 flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-between">
                                <a 
                                    href={`mailto:${selectedMember.email}`}
                                    className="inline-flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 bg-white dark:bg-[#18233c] hover:bg-slate-100 dark:hover:bg-[#223255] border border-slate-200 dark:border-slate-700 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-200 transition-colors truncate"
                                >
                                ...
                                <div className="flex gap-3 justify-end">
                                    ...
                                    {selectedMember.github && (
                                        <a 
                                            ...
                                            target="_blank" 
                                            rel="noopener noreferrer" 
                                            className="w-10 h-10 bg-slate-900 dark:bg-slate-800 hover:bg-slate-800 dark:hover:bg-slate-700 text-white rounded-xl flex items-center justify-center transition-colors"
